...
function imgOnClick(e) {
	// TODO: detect specific keystrokes.  These doesn't work so well in firefox, but shift & alt keys work OK
  	 if (e.shiftKey) {
		// this.specimen==true when it is already added
		if (this.specimen) {
			var sessionid=getCookie('id');
			if (sessionid == "") {
				alert('You are not logged in, unable to remove image');
			} else if (confirm('Do you want to disassociate this image from this specimen?')) {
				removeImage(this);
				removeMeta(this.internalid);
			}
		} else {
			var internalid=writeMeta(gURLPath+ gPicturesDir,gDirectoryPath,this.alt);
            	if (internalid==1 || !internalid ) {
               		alert('System error OR you are clicking pictures too fast. Internal ID='+internalid);
                } else if (internalid==99) {
               		alert('Invalid session or you are not logged in');
                } else {
  			    	addImage(gURLPath+ gPicturesDir+gDirectoryPath,this.alt,internalid,gDirectoryPath);
                }
		}
   	} else if (e.altKey) {
