...
$output .= "<rule type='checkInXMLFields' name='PreparationType' level='warning'>\n";
$output .= getArrayAsFields(@PreparationTypes);
$output .= "</rule>\n\n";
...
if (!$bad) {
	...
	if ($@) {
	   $bad .= "\n\nXML File Validation Error: " . $@;
	    ...
	    `cp /tmp/bioValidator.tmp /usr/local/web/biocode/web/biocodeValidator-0.9.xml`;
	}
