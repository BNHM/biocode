...
$output .= getSQLAsFields("select biocode_collecting_event.Country from biocode_collecting_event, biocode where biocode.Coll_EventID = biocode_collecting_event.EventID group by Country order by Country"); 
$output .= "</metadata>\n\n";
...
$output .= "<Specimens type='Worksheet'>\n\n";
...
$output .= getArrayAsFields(@required_specimen_fields);
$output .= "</rule>\n\n";
...
$output .= "<Collecting_Events type='Worksheet'>\n\n";
...
$output .= "<rule type='RequiredColumns' name='RequiredColumns' level='error'>\n";
$output .= getArrayAsFields(@required_collevent_fields);
$output .= "</rule>\n\n";
...
$output .= "<rule type='BoundingBox'
    name='Moorea'
    decimalLatitude='DecimalLatitude'
    decimalLongitude='DecimalLongitude' level='warning'>
...
$output .= "<rule type='BoundingBox'
    name='Moorea'
    decimalLatitude='DecimalLatitude2'
    decimalLongitude='DecimalLongitude2' level='warning'>
         <field>BOX3D(-18.5 -150.8,-16.7 -148.4)</field>
    </rule>\n\n";
...
$output .= "<rule type='validYearMonthDay' name='YearCollected,MonthCollected,DayCollected' level='error'></rule>\n\n";

$output .= "<rule type='validYearMonthDay' name='YearIdentified,MonthIdentified,DayIdentified' level='error'></rule>\n\n";
...
if (!$bad) {
	# TODO: implement email saying this was good
	open (MYFILE,'>/tmp/bioValidator.tmp');
	print MYFILE $output;
	close (MYFILE); 
	...
	use XML::SAX::ParserFactory;
  	...
  	eval { $parser->parse_uri("/tmp/bioValidator.tmp") };
  	#die "File failed validation: $@" if $@;
	if ($@) {
	   $bad .= "\n\nXML File Validation Error: " . $@;
	} else {
	    `cp /tmp/bioValidator.tmp /usr/local/web/biocode/web/biocodeValidator-0.7.xml`;
	}
