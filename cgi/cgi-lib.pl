...
# cgi-lib@pobox.com
# $Id: cgi-lib.pl,v 1.4 2002/07/03 01:04:49 joyceg Exp $
#
...
$cgi_lib'version = sprintf("%d.%02d", q$Revision: 1.4 $ =~ /(\d+)\.(\d+)/);
...
$cgi_lib'maxdata    = 2097152;    # maximum bytes to accept via POST - 2^17
...
sub ReadParse {
  ...
  if ($len > $cgi_lib'maxdata) { #'
      &CgiDie("cgi-lib.pl: Request to receive too much data: $len bytes <p>maxdata: $cgi_lib'maxdata\n");
  }
